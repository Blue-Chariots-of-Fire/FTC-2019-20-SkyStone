...
@Autonomous
public class TestAutonomousStates extends LinearOpMode
{
    ...
    private void doRedBlock ()
    {
        ...
        {
            case LEFT: drive(70, 16,true);
                break;
            case CENTER: drive (70, 0, true);
                break;
            case RIGHT: drive(70,-10,true);
                break;
            case NONE:
                break;
        }
    ...
    private void drive (double x, double y, boolean foundation)
    {
        ...
        double xStartPosition = position.getX();
        double yStartPosition = position.getY();

        double xTargetPosition = xStartPosition + x;
        double yTargetPosition = yStartPosition + y;
        double targetAngle = position.getAngle();

        double dxTotal = x!=0? xTargetPosition - xStartPosition : -200;
        double dyTotal = y!=0? yTargetPosition - yStartPosition : -200;
    ...
    private void drive (double distance, double distanceBeforeDecel)
    {
        ...
        double endPosition = yStartPosition + distanceBeforeDecel;
        ...
        if (distance > 0)
        {
            ...
            {
                turnCorrection = localization.getAngle()/(Math.PI/6);

                frontLeft.setPower(1 + turnCorrection);
                frontRight.setPower(1 - turnCorrection);
                backRight.setPower(1 + turnCorrection);
                backLeft.setPower(1 - turnCorrection);
            }
        }
        else
        {
